...
function SignUp() {
  ...
  let handleSubmit = () => {
    ...
    if (password == !confirmPass) {
      axios.post("http://localhost:5000/user_create", data)
        .then((res) => {
          navigate('/login')
        })
    }
    ...
    <>
      <div className=' flex justify-center items-center h-screen w-screen bg-colorOne'>
        <form className='flex flex-col justify-center items-center bg-colorTwo h-auto p-10 w-96 rounded-2xl shadow-2xl'>
          ...
          <input type='text' onChange={handlePassword} required className='border-2 border-black w-56 rounded-md focus:shadow-md hover:border-colorThree transition duration-300 ease ' />
          <br />

          <p className='font-semibold mr-14'>Confirm your password</p>
          <input type='text' onChange={handleConfirmPass} required className='border-2 border-black w-56 rounded-md focus:shadow-md hover:border-colorThree transition duration-300 ease ' />
          <br />
          ...
          <a href="/login" className='text-sm mt-4'>Already a User? Click <span className="text-blue-600 underline">here</span> to Login</a>
        </form>
